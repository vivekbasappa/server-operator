...
import (
	...
	appapis "github.com/example-inc/server-operator/pkg/apis"
...
func init() {
	...
	appapis.AddToScheme(scheme.Scheme)
}
...
func newHost(name string, spec *appv1alpha1.ServerSpec) *appv1alpha1.Server {
	return &appv1alpha1.Server{
		TypeMeta: metav1.TypeMeta{
			Kind:       "Server",
			APIVersion: "app.io/v1alpha1",
		},
		ObjectMeta: metav1.ObjectMeta{
			Name:      name,
			Namespace: namespace,
...
func newDefaultNamedHost(name string, t *testing.T) *appv1alpha1.Server {
	spec := &appv1alpha1.ServerSpec{
...
func newDefaultHost(t *testing.T) *appv1alpha1.Server {
	return newDefaultNamedHost(t.Name(), t)
}

func newTestReconciler(initObjs ...runtime.Object) *ReconcileServer {
...
type DoneFunc func(host *appv1alpha1.Server, result reconcile.Result) bool

func newRequest(host *appv1alpha1.Server) reconcile.Request {
...
func tryReconcile(t *testing.T, r *ReconcileServer, host *appv1alpha1.Server, isDone DoneFunc) {
	...
	for i := 0; ; i++ {
		...
		// order to test it.
		updatedHost := &appv1alpha1.Server{}
...
func waitForStatus(t *testing.T, r *ReconcileServer, host *appv1alpha1.Server, desiredStatus appv1alpha1.OperationalStatus) {
	logger := log.WithValues("host", host.ObjectMeta.Name, "desiredStatus", desiredStatus)
	tryReconcile(t, r, host,
		func(host *appv1alpha1.Server, result reconcile.Result) bool {
...
func waitForError(t *testing.T, r *ReconcileServer, host *appv1alpha1.Server) {
...
func waitForNoError(t *testing.T, r *ReconcileServer, host *appv1alpha1.Server) {
	logger := log.WithValues("host", host.ObjectMeta.Name)
	tryReconcile(t, r, host,
		func(host *appv1alpha1.Server, result reconcile.Result) bool {
...
func waitForProvisioningState(t *testing.T, r *ReconcileServer, host *appv1alpha1.Server, desiredState appv1alpha1.ProvisioningState) {
	tryReconcile(t, r, host,
		func(host *appv1alpha1.Server, result reconcile.Result) bool {
...
func TestPause(t *testing.T) {
	...
	host.Annotations = map[string]string{
		appv1alpha1.PausedAnnotation: "true",
	}
...
func TestAddFinalizers(t *testing.T) {
	...
	tryReconcile(t, r, host,
		func(host *appv1alpha1.Server, result reconcile.Result) bool {
			...
			if utils.StringInList(host.ObjectMeta.Finalizers, appv1alpha1.ServerFinalizer) {
				return true
			}
			return false
		},
...
func TestRebootWithSuffixlessAnnotation(t *testing.T) {
	...
	host.Status.Provisioning.State = appv1alpha1.StateProvisioned
	host.Spec.Online = true
	host.Spec.Image = &appv1alpha1.Image{URL: "foo", Checksum: "123"}
...
func TestDiscoveredHost(t *testing.T) {
	...
	waitForStatus(t, r, noAddress, appv1alpha1.OperationalStatusDiscovered)
	...
	waitForStatus(t, r, noAddressOrSecret, appv1alpha1.OperationalStatusDiscovered)
}
...
func TestNeedsProvisioning(t *testing.T) {
	...
	host.Spec.Image = &appv1alpha1.Image{
...
func TestExternallyProvisionedTransitions(t *testing.T) {
	...
	t.Run("registered to externally provisioned", func(t *testing.T) {
		...
		waitForProvisioningState(t, r, host, appv1alpha1.StateExternallyProvisioned)
	})
	...
	t.Run("externally provisioned to inspecting", func(t *testing.T) {
		...
		waitForProvisioningState(t, r, host, appv1alpha1.StateInspecting)
	})
	...
	t.Run("ready to externally provisioned", func(t *testing.T) {
		...
		waitForProvisioningState(t, r, host, appv1alpha1.StateReady)
...
func TestDeleteHost(t *testing.T) {
	...
	type HostFactory func() *appv1alpha1.Server

	testCases := []HostFactory{
		func() *appv1alpha1.Server {
		...
		func() *appv1alpha1.Server {
			...
			host.Spec.BMC = appv1alpha1.BMCDetails{}
			host.Finalizers = append(host.Finalizers,
				appv1alpha1.ServerFinalizer)
			return host
		},
		...
		func() *appv1alpha1.Server {
			...
			host.Status.HardwareDetails = &appv1alpha1.HardwareDetails{}
			host.Finalizers = append(host.Finalizers,
				appv1alpha1.ServerFinalizer)
			return host
		},
		func() *appv1alpha1.Server {
			...
			host.Status.HardwareDetails = &appv1alpha1.HardwareDetails{}
			host.Status.Provisioning.Image = appv1alpha1.Image{
				URL:      "image-url",
